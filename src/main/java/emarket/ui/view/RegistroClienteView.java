package emarket.ui.view;

import emarket.notificacion.CanalNotificacion;
import java.awt.GridBagConstraints;
import java.awt.GridBagLayout;
import java.awt.Insets;
import java.awt.event.ActionListener;
import java.util.ArrayList;
import java.util.List;
import javax.swing.BorderFactory;
import javax.swing.JButton;
import javax.swing.JCheckBox;
import javax.swing.JLabel;
import javax.swing.JPanel;
import javax.swing.JPasswordField;
import javax.swing.JTextField;
import javax.swing.SwingConstants;

/**
 * Vista pasiva para el alta de un Cliente. Solo recolecta datos del formulario;
 * no valida reglas de negocio ni llama a ningún servicio.
 */
public class RegistroClienteView extends JPanel {

    private final JTextField campoUsername = new JTextField(18);
    private final JPasswordField campoPassword = new JPasswordField(18);
    private final JPasswordField campoConfirmarPassword = new JPasswordField(18);
    private final JTextField campoDireccion = new JTextField(18);
    private final JTextField campoEmail = new JTextField(18);
    private final JTextField campoTelefono = new JTextField(18);
    private final JTextField campoTokenDispositivo = new JTextField(18);
    private final JCheckBox checkEmail = new JCheckBox("Email");
    private final JCheckBox checkSms = new JCheckBox("SMS");
    private final JCheckBox checkPush = new JCheckBox("Push");
    private final JButton btnRegistrar = new JButton("Registrarme");
    private final JButton btnCancelar = new JButton("Cancelar");
    private final JLabel lblMensaje = new JLabel(" ", SwingConstants.CENTER);

    public RegistroClienteView() {
        super(new GridBagLayout());
        setBorder(BorderFactory.createEmptyBorder(15, 20, 15, 20));

        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(4, 4, 4, 4);
        gbc.anchor = GridBagConstraints.WEST;
        gbc.fill = GridBagConstraints.HORIZONTAL;

        int fila = 0;
        agregarCampo(gbc, fila++, "Usuario:", campoUsername);
        agregarCampo(gbc, fila++, "Contraseña:", campoPassword);
        agregarCampo(gbc, fila++, "Confirmar contraseña:", campoConfirmarPassword);
        agregarCampo(gbc, fila++, "Dirección:", campoDireccion);
        agregarCampo(gbc, fila++, "Email:", campoEmail);
        agregarCampo(gbc, fila++, "Teléfono:", campoTelefono);
        agregarCampo(gbc, fila++, "Token dispositivo (push):", campoTokenDispositivo);

        gbc.gridx = 0; gbc.gridy = fila;
        add(new JLabel("Canales de notificación:"), gbc);
        JPanel panelCanales = new JPanel();
        panelCanales.add(checkEmail);
        panelCanales.add(checkSms);
        panelCanales.add(checkPush);
        gbc.gridx = 1; gbc.gridy = fila++;
        add(panelCanales, gbc);

        gbc.gridx = 0; gbc.gridy = fila; gbc.gridwidth = 2; gbc.anchor = GridBagConstraints.CENTER;
        lblMensaje.setForeground(java.awt.Color.RED);
        add(lblMensaje, gbc);
        fila++;

        JPanel panelBotones = new JPanel();
        panelBotones.add(btnRegistrar);
        panelBotones.add(btnCancelar);
        gbc.gridx = 0; gbc.gridy = fila; gbc.gridwidth = 2;
        add(panelBotones, gbc);
    }

    private void agregarCampo(GridBagConstraints gbc, int fila, String etiqueta, JTextField campo) {
        gbc.gridwidth = 1;
        gbc.gridx = 0; gbc.gridy = fila;
        add(new JLabel(etiqueta), gbc);
        gbc.gridx = 1;
        add(campo, gbc);
    }

    public String getUsername() { return campoUsername.getText().trim(); }
    public String getPassword() { return new String(campoPassword.getPassword()); }
    public String getConfirmarPassword() { return new String(campoConfirmarPassword.getPassword()); }
    public String getDireccion() { return campoDireccion.getText().trim(); }
    public String getEmail() { return campoEmail.getText().trim(); }
    public String getTelefono() { return campoTelefono.getText().trim(); }
    public String getTokenDispositivo() { return campoTokenDispositivo.getText().trim(); }

    public List<CanalNotificacion> getCanalesSeleccionados() {
        List<CanalNotificacion> canales = new ArrayList<>();
        if (checkEmail.isSelected()) canales.add(CanalNotificacion.EMAIL);
        if (checkSms.isSelected()) canales.add(CanalNotificacion.SMS);
        if (checkPush.isSelected()) canales.add(CanalNotificacion.PUSH);
        return canales;
    }

    public void limpiarCampos() {
        campoUsername.setText("");
        campoPassword.setText("");
        campoConfirmarPassword.setText("");
        campoDireccion.setText("");
        campoEmail.setText("");
        campoTelefono.setText("");
        campoTokenDispositivo.setText("");
        checkEmail.setSelected(false);
        checkSms.setSelected(false);
        checkPush.setSelected(false);
        lblMensaje.setText(" ");
    }

    public void mostrarError(String mensaje) {
        lblMensaje.setForeground(java.awt.Color.RED);
        lblMensaje.setText(mensaje);
    }

    public void mostrarMensaje(String mensaje) {
        lblMensaje.setForeground(new java.awt.Color(0, 128, 0));
        lblMensaje.setText(mensaje);
    }

    public void setRegistrarListener(ActionListener listener) {
        btnRegistrar.addActionListener(listener);
    }

    public void setCancelarListener(ActionListener listener) {
        btnCancelar.addActionListener(listener);
    }
}
